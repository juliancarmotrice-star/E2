...
# Configuración de rutas relativas y absolutas independientes de dónde se ejecute el script
...
def get_raw_path(filename):
    """Busca el archivo en datos_iniciales o en la ruta directa si existe."""
...
def run_cleaning_pipeline():
    ...
    oc_raw = pd.read_csv(get_raw_path('ordenes_compra.csv'))
    ...
    mm.to_csv(os.path.join(CLEAN_DIR, 'maestro_materiales_clean.csv'), index=False)
    ...
    mov.to_csv(os.path.join(CLEAN_DIR, 'movimientos_inventario_clean.csv'), index=False)
    ...
    oc.to_csv(os.path.join(CLEAN_DIR, 'ordenes_compra_clean.csv'), index=False)
    ...
    # =========================================================================
    bom_raw = pd.read_csv(get_raw_path('bom.csv'))
    plan_raw = pd.read_csv(get_raw_path('plan_produccion.csv'))
    ...
    maestro_prod.to_csv(os.path.join(CLEAN_DIR, 'maestro_productos_clean.csv'), index=False)
    ...
    bom.to_csv(os.path.join(CLEAN_DIR, 'bom_clean.csv'), index=False)
    ...
    plan.to_csv(os.path.join(CLEAN_DIR, 'plan_produccion_clean.csv'), index=False)
    ...
    ii.to_csv(os.path.join(CLEAN_DIR, 'inventario_inicial_clean.csv'), index=False)
    ...
    # Deduplicación (preservar primera ocurrencia si existen duplicados)
    num_dups = cf.duplicated(subset=['sku']).sum()
    if num_dups > 0:
        cf = cf.drop_duplicates(subset=['sku'], keep='first')
        
    # Corrección de signos negativos: interpretación como error de digitación (valor absoluto)
    ...
    cf_clean.to_csv(os.path.join(CLEAN_DIR, 'conteo_fisico_clean.csv'), index=False)
    ...
    jefe_clean.to_csv(os.path.join(CLEAN_DIR, 'inventario_bodega_JEFE_clean.csv'), index=False)
