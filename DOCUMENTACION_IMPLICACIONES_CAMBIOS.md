...
| **6** | `conteo_fisico.csv` | 94 SKUs con stock físico contado negativo (hasta -18.018 unidades). | Imputación analítica mediante reconstrucción de saldo teórico con la "Tabla Madre" e historial Kardex. | Un stock físico no puede ser menor a cero; los negativos evidencian desfase transaccional (consumo en planta antes de formalizar entrada). | Restablece saldos físicamente coherentes para el cálculo de exactitud de registros (IRA) y auditoría de inventario. |
...
| **8** | `Inventario_bodega_JEFE.csv` | 32 negativos; 23 notas vacías; vocabulario informal (`faltante??`, `sobra`). | Definición de rol como **fuente de verificación paralela y auditoría de piso**; truncado a $\ge 0$; notas imputadas. | No reemplaza el Kardex oficial, pero diagnostica la "libreta paralela" creada por falta de credibilidad en el ERP. | Permite medir cuantitativamente la brecha entre el ERP y el piso (Exactitud de Registro IRA = 28.33%). |
